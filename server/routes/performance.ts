...
const GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwmJKq8bSLbjcrgClRrLizCcZuftV9DmKsTpJxBoQ63THrtmkjV5UQd0WVDZnw51tytXg/exec";
...
const tryFetch = async (url: string, attempt: number = 1): Promise<Response | null> => {
  ...
  try {
    ...
    timeout = setTimeout(() => {
      console.warn(`Attempt ${attempt}: Request timed out after 120 seconds`);
      controller.abort();
    }, 120000);
...
export const handlePerformanceData: RequestHandler = async (req, res) => {
  try {
    console.log("\n╔════════════════════════════════════════════════════╗");
    console.log("║  Starting Performance Data Fetch Request           ║");
    console.log("╚════════════════════════════════════════════════════╝");
    ...
    console.error("\n✗ EXCEPTION in handlePerformanceData:");
    console.error(error);
    res.status(500).json({
      error: "Server error",
